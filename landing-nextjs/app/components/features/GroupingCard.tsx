...
const CUSTOMERS = [
    { name: "Marie L.", tag: "VIP", color: "bg-amber-100 text-amber-700" },
    ...
    return (
        <div className="max-w-md mx-auto p-4">
            ...
            >
                <div
                    ref={containerRef}
                    className="relative bg-gradient-to-br from-indigo-500 to-violet-700 p-8 flex-1 flex items-center justify-center overflow-hidden"
                >
                    ...
                    <div className="w-full max-w-[280px] bg-white rounded-2xl shadow-xl p-5 relative z-10 border border-white/60">
                        <div className="h-[200px] flex flex-col">
                            <div className="flex items-center justify-between mb-4">
                                <span className="text-[10px] font-bold uppercase tracking-wider text-slate-400">Audience</span>
                            ...
                            <div className="space-y-2 flex-1">
                                {CUSTOMERS.map((customer, idx) => (
                                    <motion.div
                                        ...
                                        className={`flex items-center justify-between p-2 rounded-xl border ${isFiltered && customer.tag === "VIP" ? 'border-indigo-200 bg-indigo-50/50' : 'border-slate-50 bg-slate-50/50'}`}
                                    >
                                        <div className="flex items-center gap-2">
                                            <div className="w-6 h-6 rounded-full bg-slate-200 flex items-center justify-center text-[10px] font-bold text-slate-500">
                                                {customer.name[0]}
                                            </div>
                                            <span className="text-[11px] font-medium text-slate-700">{customer.name}</span>
                                        </div>
                                        <span className={`text-[9px] px-2 py-0.5 rounded-full font-bold ${customer.color}`}>
                                            {customer.tag}
                                        </span>
                                    </motion.div>
                                ))}
                        ...
                        <div className="mt-5 relative">
                            <motion.div
                                ...
                                className={`
                                    ...
                                    ${phase === "filtering" ? "bg-indigo-950" : ""}
                                    ${phase === "done" ? "bg-green-600 shadow-green-700/30" : "bg-indigo-900 shadow-indigo-900/20"}
                                `}
                            >
                                {phase === "done" ? <Check className="w-4 h-4" /> : <Filter className="w-4 h-4" />}
                                <span>
                                    {phase === "filtering" ? `Ciblage: ${TARGET_GROUPS[selectedGroup]}` :
                                        phase === "done" ? "Groupe VIP Ciblé" : "Cibler mes VIP"}
                                </span>
                            </motion.div>
                        </div>
                    </div>
                ...
                <div className="p-8 bg-white border-t border-slate-50 text-left">
                    <div className="flex items-center gap-3 mb-3">
                        <div className="bg-indigo-50 p-2 rounded-lg">
                            <Target className="text-indigo-900 w-5 h-5" />
                        </div>
                        <h3 className="text-lg font-bold text-slate-900 tracking-tight">Ciblage Client Précis</h3>
                    </div>
                    <p className="text-slate-500 text-sm leading-relaxed">
                        Ne parlez qu&apos;aux bons clients. <strong className="text-indigo-900 font-semibold">Créez des groupes intelligents</strong> pour multiplier vos conversions par 3 avec des offres ultra-personnalisées.
                    </p>
                </div>
            </motion.div>
        </div>
